...
import { Loader2, Plus, Trash2, FileCheck2, Send, Printer, ShieldCheck, CheckCircle2 } from 'lucide-react';
...
const STATUS: Record<string, { label: string; cls: string }> = {
  ...
  issued: { label: 'Émise', cls: 'bg-sky-500/15 text-sky-400' },
...
const blankLine = (): Line => ({ _k: ++lk, description: '', quantity: 1, unit_price: 0, vat_rate: 0.18, account_code: '706' });
...
export default function Facturation({ dossierId, dossierName, currency }: { dossierId: string; dossierName: string; currency: string }) {
  ...
  const load = async () => { setLoading(true); try { setRows(await api.invoices(dossierId)); } finally { setLoading(false); } };
  ...
  const create = async (e: React.FormEvent) => {
    ...
    setBusy('create');
    try {
      await api.createInvoice(dossierId, { clientName: client.trim(), invoiceDate: date, dueDate: due || undefined, lines: lines.map((l) => ({ description: l.description, quantity: Number(l.quantity), unit_price: Number(l.unit_price), vat_rate: Number(l.vat_rate), account_code: l.account_code })) });
      setCreating(false); setClient(''); setDue(''); setLines([blankLine()]); await load();
    } catch (e: any) { setError(e.message); } finally { setBusy(null); }
  };

  const act = async (fn: () => Promise<any>, key: string) => { setBusy(key); setError(null); try { await fn(); await load(); } catch (e: any) { setError(e.message); } finally { setBusy(null); } };
  ...
  const printInvoice = async (id: string) => {
    ...
    const body = `
      <table style="margin-bottom:12px"><tbody>
        <tr><td><b>FACTURE ${inv.number ?? '(brouillon)'}</b></td><td class="n">Date : ${inv.invoice_date}</td></tr>
        <tr><td>Fournisseur : ${dossierName}</td><td class="n">${inv.due_date ? 'Échéance : ' + inv.due_date : ''}</td></tr>
      ...
      <tr class="tot"><td colspan="4">Total TTC</td><td class="n">${m(inv.total_ttc)}</td></tr>
      </tbody></table>
      ${inv.fne_reference ? `<p style="margin-top:12px"><b>Facture Normalisée Électronique</b> — Réf. ${inv.fne_reference}</p>` : ''}`;
    printDocument(`Facture ${inv.number ?? ''} — ${dossierName}`, `édité le ${nowStamp()}`, body);
  };

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-zinc-400">Factures de vente</p>
        <button onClick={() => setCreating((v) => !v)} className="flex items-center gap-2 rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-zinc-950 hover:bg-emerald-400"><Plus className="h-4 w-4" /> Nouvelle facture</button>
      </div>
      ...
      {creating && (
        <motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} onSubmit={create} className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5">
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div><label className="mb-1 block text-xs text-zinc-500">Échéance</label><input type="date" value={due} onChange={(e) => setDue(e.target.value)} className="w-full rounded-lg border border-white/10 bg-zinc-900/60 px-3 py-2 text-sm outline-none focus:border-emerald-500/50" /></div>
          </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <button type="submit" disabled={busy === 'create'} className="flex items-center gap-2 rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-zinc-950 hover:bg-emerald-400 disabled:opacity-50">{busy === 'create' && <Loader2 className="h-4 w-4 animate-spin" />} Créer la facture</button>
          </div>
        </motion.form>
      )}
      ...
      {loading ? <div className="flex items-center gap-2 text-zinc-400"><Loader2 className="h-4 w-4 animate-spin" /> Chargement…</div> : rows.length === 0 ? <p className="text-sm text-zinc-500">Aucune facture. Créez la première.</p> : (
            ...
            <tbody className="divide-y divide-white/5">
              {rows.map((inv) => (
                <tr key={inv.id} className="hover:bg-white/5">
                  ...
                  <td className="px-4 py-2.5 text-right font-mono text-zinc-200">{fmtMoney(inv.total_ttc, currency)}</td>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center justify-end gap-3">
                      {inv.status === 'draft' && <button onClick={() => act(() => api.issueInvoice(dossierId, inv.id), 'i' + inv.id)} className="flex items-center gap-1 text-xs text-sky-400 hover:text-sky-300">{busy === 'i' + inv.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />} Émettre</button>}
                      {inv.status !== 'draft' && inv.fne_status !== 'certified' && <button onClick={() => act(() => api.certifyInvoice(dossierId, inv.id), 'c' + inv.id)} className="flex items-center gap-1 text-xs text-emerald-400 hover:text-emerald-300">{busy === 'c' + inv.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FileCheck2 className="h-3.5 w-3.5" />} Certifier FNE</button>}
                      <button onClick={() => printInvoice(inv.id)} className="text-zinc-500 hover:text-zinc-300"><Printer className="h-4 w-4" /></button>
