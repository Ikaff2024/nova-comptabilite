...
import { Loader2, Sparkles, Send, Wrench, User } from 'lucide-react';
...
export default function Assistant({ dossierId, dossierName }: { dossierId: string; dossierName: string; currency: string }) {
  ...
  useEffect(() => { api.agentStatus(dossierId).then((s) => setEnabled(s.enabled)).catch(() => setEnabled(false)); }, [dossierId]);
    ...
    <div className="flex h-[70vh] flex-col rounded-2xl border border-white/10 bg-white/5">
      <div className="flex items-center gap-2 border-b border-white/10 px-4 py-3">
        ...
        <div>
          ...
          <div className="text-xs text-zinc-500">Pilotez {dossierName} en langage naturel — lecture seule</div>
        </div>
      </div>
              ...
              {t.tools && t.tools.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {[...new Set(t.tools)].map((name: string) => (
                    <span key={name} className="inline-flex items-center gap-1 rounded-full bg-white/5 px-2 py-0.5 text-[11px] text-zinc-400"><Wrench className="h-3 w-3" /> {TOOL_LABELS[name] ?? name}</span>
                  ))}
                </div>
              )}
      ...
      <p className="px-4 pb-3 text-center text-[11px] text-zinc-600">L'assistant lit vos données comptables mais ne saisit rien. Vérifiez toujours avant décision.</p>
    </div>
  );
}
