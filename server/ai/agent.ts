...
import * as behavior from '../domain/behavior.js';
...
const READ_TOOLS = [
  ...
  { name: 'balance_auxiliaire', description: "Balance auxiliaire des tiers : chaque client/fournisseur (et autres tiers) avec ses totaux débit/crédit et son solde débiteur ou créditeur, regroupé par nature. Sert à JUSTIFIER les comptes collectifs (411, 401) et à faire le point sur les soldes tiers. On peut filtrer par nature (type : client | fournisseur | salarie | etat | autre).", input_schema: { type: 'object', properties: { type: { type: 'string', description: 'client | fournisseur | salarie | etat | autre (optionnel)' } }, required: [] } },
  ...
  { name: 'rapport_activite', description: "Rapport d'ACTIVITÉ mensuel (digest d'employée de Lexa) : ce qui a été fait dans le mois (écritures comptabilisées, factures de vente/achat, bulletins de paie), ce que Lexa a fait (actions et échanges tracés), les chiffres clés (produits, charges, résultat, trésorerie), les points À TRAITER (alertes priorisées) et des recommandations. Pour « fais le bilan du mois » / « qu'est-ce qui s'est passé ce mois ». Le PDF s'envoie par email via pieces_jointes = [{ document: \"rapport_activite\" }]. Fournir année et mois (1-12).", input_schema: { type: 'object', properties: { annee: { type: 'number' }, mois: { type: 'number', description: 'Mois en clair 1-12' } }, required: [] } },
  ...
  { name: 'valider_declaration', description: "AQM — Contrôle qualité DÉTERMINISTE d'une DÉCLARATION avant dépôt : TVA (cohérence avec le régime — synthétique = non assujetti, période non clôturée, ventes vs TVA collectée, résultat), ou paie CNPS / DGI (bulletins présents et cotisations/impôts calculés). Verdict PASS/WARNING/FAIL + score + raisons. UTILISE-LE avant de proposer/produire une déclaration ; si FAIL, ne la produis pas. Paramètres : type (tva|cnps|dgi), annee, mois (1-12).", input_schema: { type: 'object', properties: { type: { type: 'string', description: 'tva | cnps | dgi' }, annee: { type: 'number' }, mois: { type: 'number', description: '1-12' } }, required: ['type'] } },
  { name: 'simuler_ecriture', description: "Simulation AVANT validation : à partir des lignes d'un brouillon, calcule l'IMPACT (sans rien comptabiliser) sur le résultat, la TVA à payer, la trésorerie et l'IS estimé (indicatif 25 %). Pour « qu'est-ce que ça change si je passe cette écriture / facture ». Complète valider_ecriture (qui, lui, contrôle la conformité). Fournir les lignes (compte, debit, credit).", input_schema: { type: 'object', properties: { lignes: { type: 'array', items: { type: 'object', properties: { compte: { type: 'string' }, debit: { type: 'number' }, credit: { type: 'number' } }, required: ['compte'] } } }, required: ['lignes'] } },
  ...
  { name: 'tableau_flux_tresorerie', description: "Tableau de flux de trésorerie (TFT, SYSCOHADA révisé), méthode INDIRECTE SIMPLIFIÉE, reconstitué à partir des grandes masses N et N-1 : flux opérationnels (résultat + dotations − variation du BFR), flux d'investissement (acquisitions nettes d'immobilisations), flux de financement (Δ capitaux propres + Δ dettes financières), variation calculée, ÉCART DE RÉCONCILIATION et variation constatée de trésorerie (ouverture → clôture). Pour expliquer « où est passée la trésorerie ? ». INDICATIF : ne capte pas finement dividendes/capital/affectation du résultat (c'est l'écart) — précise-le. Nécessite un exercice N-1.", input_schema: { type: 'object', properties: {}, required: [] } },
  { name: 'signaux_habitudes', description: "Détection comportementale : écarts aux HABITUDES du dossier (un fournisseur habituellement mensuel sans écriture ce mois, un rythme de saisie nettement en baisse). Pour « quelque chose d'inhabituel ? » ou une veille proactive. Conservateur : ne remonte que des écarts nets, à vérifier.", input_schema: { type: 'object', properties: {}, required: [] } },
  { name: 'conseils_fiscaux', description: "Assistant fiscal proactif : pistes de vigilance et d'optimisation dérivées de la comptabilité et du régime (IS vs IMF, déficit reportable, acomptes à provisionner, crédit de TVA à se faire rembourser, amendes/pénalités non déductibles à réintégrer, régime fiscal incohérent avec le CA). Pour « comment optimiser ma fiscalité », « à quoi faire attention fiscalement ». Présente-les comme des PISTES à valider avec un fiscaliste, jamais comme un conseil définitif.", input_schema: { type: 'object', properties: {}, required: [] } },
      ...
      properties: {
        ...
        pieces_jointes: {
          ...
          items: {
            type: 'object',
            properties: {
              document: { type: 'string', description: '"livre_paie" | "ordre_virement" | "courrier_virement" (lettre à la banque) | "bulletin" | "attestation_travail" (atteste l\'emploi d\'un salarié, préciser salarie) | "attestation_salaire" (atteste emploi + rémunération brute/nette, préciser salarie) | "etat_301" (état nominatif annuel des salaires — récap DGI de l\'exercice, préciser annee) | "declaration_cnps" | "declaration_dgi" | "declaration_tva" | "recap_tva" (récapitulatif annuel de TVA sur 12 mois, préciser annee) | "rapport_mensuel" | "rapport_activite" (rapport d\'activité mensuel de Lexa : activité, à traiter, chiffres, recommandations) | "balance" | "grand_livre" (un compte précis, préciser compte) | "grand_livre_general" (tous les comptes) | "journal_centralisateur" (récap mensuel par journal) | "etats_financiers" | "etats_comparatifs" (bilan & résultat comparés N vs N-1, variation) | "tft" (tableau de flux de trésorerie, simplifié, nécessite N-1) | "liasse_fiscale" (liasse fiscale / DSF SYSCOHADA complète produite par le moteur AuthNTIC — Bilan, CR, TFT) | "livre_journal" | "releve_tiers" (relevé de compte d\'un client/fournisseur, préciser tiers) | "etat_immobilisations" | "etat_rapprochement" (rapprochement bancaire, préciser compte ex. 521) | "confirmation_solde" (lettre de confirmation de solde à un tiers, préciser tiers) | "balance_agee" (balance âgée des créances/dettes par tiers) | "balance_auxiliaire" (balance des comptes de tiers, justifie 411/401) | "grand_livre_auxiliaire" (détail ligne à ligne de tous les tiers)' },
              ...
              salarie: { type: 'string', description: 'Pour "bulletin" / "attestation_travail" / "attestation_salaire" : matricule ou nom du salarié' },
...
async function executeTool(c: Client, dossierId: string, fyId: string | null, name: string, input: any, mode: AgentMode): Promise<any> {
  ...
  switch (name) {
    ...
    case 'rapport_activite': { const y = Number(input?.annee) || new Date().getUTCFullYear(); const mo = clampMonth(input?.mois); return await activityreport.activityReport(c, dossierId, y, mo); }
    ...
    case 'signaux_habitudes': return await behavior.behaviorSignals(c, dossierId);
    case 'conseils_fiscaux': return await fiscaladvisor.fiscalAdvisor(c, dossierId, fy);
    case 'estimation_is': return await tax.estimationIS(c, dossierId, fy);
    case 'tableau_flux_tresorerie': { const t = await acc.cashFlowStatement(c, dossierId, fy); if (!t.hasPrevious) return { note: 'Le TFT compare deux exercices : aucun exercice N-1 avec mouvements disponible.' }; return t; }
    ...
    case 'simuler_ecriture': {
      const lignes = Array.isArray(input?.lignes) ? input.lignes : [];
      const draft = lignes.map((l: any) => ({ accountCode: String(l.compte ?? '').trim(), debit: Number(l.debit) || 0, credit: Number(l.credit) || 0 }));
      return await simulate.simulateEntry(c, dossierId, draft);
    }
    case 'valider_declaration': {
      const y = Number(input?.annee) || new Date().getUTCFullYear();
      const mo = clampMonth(input?.mois);
      const type = String(input?.type ?? 'tva');
      if (type === 'cnps' || type === 'dgi') return await aqm.validatePayrollDeclaration(c, dossierId, y, mo, type as 'cnps' | 'dgi');
      ...
      return await aqm.validateVatDeclaration(c, dossierId, from, to);
    }
...
async function buildDocAttachment(c: Client, dossierId: string, fy: string | undefined, pj: any): Promise<{ filename: string; buffer: Buffer } | { error: string; extra?: any }> {
  ...
  switch (pj.document) {
    ...
    case 'attestation_travail': case 'attestation_salaire': { const r = await payroll.attestationPdf(c, dossierId, String(pj.salarie ?? ''), pj.document === 'attestation_salaire' ? 'salaire' : 'travail'); if (!r.found) return { error: `Salarié « ${pj.salarie} » introuvable pour l'attestation.`, extra: { salaries_disponibles: r.candidates } }; return r; }
    case 'etat_301': { const r = await payroll.etatAnnuelSalairesPdf(c, dossierId, y); if (r.count === 0) return { error: `Aucun bulletin comptabilisé pour ${y} : l'état annuel des salaires est vide.` }; return r; }
    case 'rapport_mensuel': return await reporting.rapportMensuelPdf(c, dossierId, y, mo);
    case 'rapport_activite': return await activityreport.activityReportPdf(c, dossierId, y, mo);
